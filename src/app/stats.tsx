...
import type { DietLog, MacroTargets, Rank, RoastEntry, UserProfile, WeightEntry, WorkoutLog } from '@/types';
...
const MUSCLE_KEYWORDS: Record<string, string[]> = {
  ...
  Back:       ['row', 'pull up', 'pullup', 'pull-up', 'lat pulldown', 'lat pull', 'back', 'deadlift', 'chin up', 'chin-up'],
  Shoulders:  ['shoulder', 'ohp', 'overhead press', 'lateral raise', 'front raise', 'delt'],
  Biceps:     ['curl', 'bicep', 'hammer', 'preacher'],
  Triceps:    ['tricep', 'extension', 'pushdown', 'skull crusher', 'close grip', 'dip'],
  Quads:      ['squat', 'leg press', 'quad', 'leg extension', 'lunge', 'hack squat', 'bulgarian'],
  Hamstrings: ['romanian', 'hamstring', 'leg curl', 'rdl', 'stiff leg', 'nordic'],
  Glutes:     ['hip thrust', 'glute bridge', 'kickback', 'abduction'],
  Core:       ['ab', 'core', 'plank', 'crunch', 'sit up', 'situp', 'sit-up', 'russian twist'],
};

const MUSCLE_TARGETS: Record<string, number> = {
  Chest: 16, Back: 16, Shoulders: 12, Biceps: 10,
  Triceps: 10, Quads: 16, Hamstrings: 12, Glutes: 12, Core: 8,
};
...
function weeklyVolumeData(logs: WorkoutLog[], numWeeks: number): ChartPoint[] {
  ...
  return Array.from({ length: numWeeks }, (_, i) => {
    ...
    return { value: vol, label: wStart.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) };
  });
}
...
function dailyVolumeData(logs: WorkoutLog[]): ChartPoint[] {
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    const vol = logs.filter((l) => dayKey(l.date) === k).reduce((sum, l) => sum + logVolume(l), 0);
    return { value: vol, label: DLABELS[i] };
  });
}

interface HeatCell { date: number; compliance: number; calories: number; protein: number; }
interface HeatRow  { label: string; cells: HeatCell[]; }
...
function buildHeatmap(dietLogs: DietLog[], targets: MacroTargets): HeatRow[] {
  ...
  return Array.from({ length: 4 }, (_, row) => {
    ...
    const cells: HeatCell[] = Array.from({ length: 7 }, (__, d) => {
      ...
      return {
        date: day.getTime(),
        compliance: isFuture ? -2 : log ? computeDayCompliance(log, targets) : -1,
        calories: log?.totalCalories ?? 0,
        protein: log?.totalProteinG ?? 0,
      };
    });
    return { label: wStart.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }), cells };
  });
}
...
interface LiftPoint extends ChartPoint { reps: number; weightKg: number; }

function liftProgressionData(logs: WorkoutLog[], exercise: string): LiftPoint[] {
  ...
  return [...logs]
    ...
    .map((session) => {
      const sets = session.exercises.filter((e) => e.exercise.toLowerCase() === ex);
      const top = sets.reduce((m, s) => (s.weightKg > m.weightKg ? s : m));
      const isPR = top.weightKg > prWeight;
      if (isPR) prWeight = top.weightKg;
      return { value: top.weightKg, label: fmtShort(session.date), isPR, reps: top.reps, weightKg: top.weightKg };
    });
}
...
function topExercises(logs: WorkoutLog[]): string[] {
  ...
  const byFreq = [...counts.entries()].sort((a, b) => b[1] - a[1]).map(([n]) => n);
  const defaults = ['Bench Press', 'Squat', 'Deadlift', 'OHP'];
  for (const d of defaults)
    if (!byFreq.some((n) => n.toLowerCase() === d.toLowerCase())) byFreq.push(d);
...
interface PREntry { exercise: string; weightKg: number; reps: number; date: number; isNew: boolean; }

function computePRs(logs: WorkoutLog[]): PREntry[] {
  const prMap = new Map<string, { weightKg: number; reps: number; date: number }>();
  for (const l of [...logs].sort((a, b) => a.date - b.date))
    for (const e of l.exercises) {
      const cur = prMap.get(e.exercise);
      if (!cur || e.weightKg > cur.weightKg)
        prMap.set(e.exercise, { weightKg: e.weightKg, reps: e.reps, date: l.date });
    }
...
function consistencyGrid(workoutLogs: WorkoutLog[], dietLogs: DietLog[]): DayState[][] {
  const monday = getMondayOfWeek(new Date());
  ...
  return Array.from({ length: 12 }, (_, week) => {
    ...
    return Array.from({ length: 7 }, (__, day) => {
      ...
      const k = dayKey(d.getTime());
      return (wDays.has(k) && dDays.has(k) ? 2 : wDays.has(k) || dDays.has(k) ? 1 : 0) as DayState;
    });
  });
}
    ...
    <View style={[s.rankCard, { borderColor: `${rankColor}35` }]}>
      ...
      <View style={s.rankCardInner}>
        <View style={s.rankLeft}>
          ...
          <BrutlText style={[s.rankTitle, { color: rankColor }]}>{RANK_TITLES[rank]}</BrutlText>
        </View>
        <View style={s.rankRight}>
          <BrutlText style={s.rankXPNum}>{profile.xp.toLocaleString()}</BrutlText>
          <BrutlText style={s.rankXPLabel}>total XP</BrutlText>
          {profile.streakDays > 0 ? (
            <BrutlText style={s.rankStreak}>
              🔥 {profile.streakDays} {profile.streakDays === 1 ? 'day' : 'days'} streak
            </BrutlText>
          ) : (
            <BrutlText style={[s.rankStreak, { color: BrutlColors.textDisabled }]}>No streak yet.</BrutlText>
          )}
        </View>
      </View>
      ...
      <View style={{ gap: 5, marginTop: 14 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <BrutlText style={s.xpBarLabel}>{rank} · {RANK_TITLES[rank]}</BrutlText>
          {nextRank ? (
            <BrutlText style={s.xpBarLabel}>
              {(getXPForNextRank(rank) - profile.xp).toLocaleString()} XP to {RANK_TITLES[nextRank]}
            </BrutlText>
          ) : (
            <BrutlText style={[s.xpBarLabel, { color: rankColor }]}>MAX RANK</BrutlText>
          )}
        </View>
        <View style={s.xpTrack}>
          <View style={[s.xpFill, { width: `${(pct * 100).toFixed(1)}%` as any }]} />
        </View>
        {profile.xp === 0 && (
...
function OverviewGrid({
  ...
  const weekCompliance =
    weekDiet.length > 0
      ? Math.round(
          (weekDiet.reduce((s, l) => s + computeDayCompliance(l, profile.macroTargets), 0) /
            weekDiet.length) * 100,
        )
      : 0;
  const compColor =
    weekCompliance >= 80 ? BrutlColors.success
    : weekCompliance >= 60 ? BrutlColors.warning
    : weekCompliance > 0 ? BrutlColors.accent
    : BrutlColors.textDisabled;

  const volDisplay =
    totalVolume >= 1_000_000 ? `${(totalVolume / 1_000_000).toFixed(1)}M`
    : totalVolume >= 1_000 ? `${Math.round(totalVolume / 1_000)}k`
    : Math.round(totalVolume).toString();

  const tiles = [
    { value: totalWorkouts.toString(), label: totalWorkouts === 1 ? 'session' : 'sessions', color: BrutlColors.textPrimary },
    { value: profile.streakDays.toString(), label: profile.streakDays === 1 ? 'day streak' : 'days streak', color: BrutlColors.accent },
    { value: volDisplay, label: 'kg moved', color: BrutlColors.textPrimary },
    { value: weekCompliance > 0 ? `${weekCompliance}%` : '—', label: 'this week', color: compColor },
  ];

  const goalLabel = (profile.goal ?? '—').replace('_', ' ');
  const weakLabel = profile.weakArea.map((w) => w.replace(/_/g, ' ')).join(', ') || '—';
    ...
    <BrutlCard>
      ...
      <View style={s.tileGrid}>
        {tiles.map((t, i) => (
          <View key={i} style={s.tile}>
            <BrutlText style={[s.tileValue, { color: t.color }]}>{t.value}</BrutlText>
      ...
      <View style={{ marginTop: 14, gap: 7 }}>
        {([
          ...
          watchConnected ? { k: 'Watch Connected', v: 'Health Connect', dot: true } : null,
...
function VolumeTrend({ workoutLogs }: { workoutLogs: WorkoutLog[] }) {
  ...
  const data =
    range === '1W' ? dailyVolumeData(workoutLogs)
    : range === '4W' ? weeklyVolumeData(workoutLogs, 4)
        ...
        right={
          <View style={{ flexDirection: 'row', gap: 4 }}>
            {(['1W', '4W', '3M'] as const).map((r) => (
              ...
              >
                <BrutlText style={[s.rangePillText, range === r && s.rangePillTextActive]}>{r}</BrutlText>
              </TouchableOpacity>
            ))}
          </View>
        }
      ...
      {hasData && (
        <View style={s.annotRow}>
          <BrutlText style={s.annotLeft}>
            {range === '1W' ? 'This week' : 'Latest'}: {Math.round(lastVal).toLocaleString()} kg
          </BrutlText>
          {pctChange !== null && prevVal > 0 && (
            <BrutlText style={[s.annotRight, { color: up ? BrutlColors.success : BrutlColors.accent }]}>
...
function MacroHeatmap({ dietLogs, targets }: { dietLogs: DietLog[]; targets: MacroTargets }) {
  ...
  return (
    <BrutlCard>
      ...
      <View style={s.hmDayHeader}>
        <View style={{ width: 38 }} />
        {DAY_LABELS.map((d, i) => (
          <BrutlText key={i} style={s.hmDayLabel}>{d}</BrutlText>
        ))}
      </View>
      {rows.map((row, ri) => (
        <View key={ri} style={s.hmRow}>
          ...
          {row.cells.map((cell, ci) => (
            <TouchableOpacity
              key={ci}
              onPress={() => setTooltip(tooltip?.date === cell.date ? null : cell)}
              activeOpacity={0.75}
            >
              <View
                style={[
      ...
      {dietLogs.length === 0 && (
        <BrutlText style={[s.emptyNote, { marginTop: 10 }]}>
          Log your first meal to start filling this in. Right now it just looks like a void.
        </BrutlText>
      )}
    </BrutlCard>
  );
...
function BodyWeightSection({
  ...
  const chartData: ChartPoint[] = [...entries].reverse().slice(0, 56).map((e) => ({
    value: e.weightKg,
  ...
  const change = current != null && oldest != null && entries.length > 1 ? current - oldest : null;
  ...
  return (
    <BrutlCard>
      ...
      {entries.length < 2 ? (
        <BrutlText style={s.emptyNote}>
          You haven't logged your weight yet. Avoiding the number doesn't change it.
        </BrutlText>
      ) : (
        <>
          <LineChart
          ...
          />
          <View style={s.annotRow}>
            <BrutlText style={s.annotLeft}>Current: {current?.toFixed(1)} kg</BrutlText>
            {change !== null && (
              <BrutlText style={[s.annotRight, { color: change <= 0 ? BrutlColors.success : BrutlColors.warning }]}>
          ...
          <View style={s.weightStatRow}>
            {[
              ...
              { label: 'Change', value: change !== null ? `${change > 0 ? '+' : ''}${change.toFixed(1)} kg` : '—' },
        ...
        {loggedToday ? (
          ...
          <View style={s.weightInputRow}>
            <TextInput
              ...
              autoFocus
            />
            <BrutlText style={{ color: BrutlColors.textMuted, fontSize: 13 }}>kg</BrutlText>
            <TouchableOpacity onPress={confirm}>
              <BrutlText style={{ color: BrutlColors.accent, fontSize: 13 }}>Confirm</BrutlText>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => { setShowInput(false); setInput(''); }}>
              <BrutlText style={{ color: BrutlColors.textMuted, fontSize: 13 }}>Cancel</BrutlText>
            </TouchableOpacity>
          </View>
        ) : (
    ...
    <BrutlCard>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {exercises.map((ex) => (
            ...
            >
              <BrutlText style={[s.exPillText, selected === ex && s.exPillTextActive]}>{ex}</BrutlText>
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
      ...
      {data.length > 0 && (
        <View style={{ gap: 3, marginTop: 10 }}>
          ...
          {allTimePR && (
            <BrutlText style={s.annotLeft}>
              All-time PR: {allTimePR.weightKg} kg × {allTimePR.reps} · {allTimePR.label}
            </BrutlText>
          )}
        </View>
      )}
...
function MuscleVolume({ workoutLogs }: { workoutLogs: WorkoutLog[] }) {
  ...
  const weekSessions = workoutLogs.filter((l) => l.date >= weekStart.getTime()).length;
    ...
    <BrutlCard>
      ...
      <View style={{ gap: 10 }}>
        {MUSCLE_ORDER.map((muscle) => {
          ...
          const fillColor =
            ratio <= 0 ? BrutlColors.borderVisible
            : ratio < 0.31 ? '#2A0000'
            : ratio < 0.71 ? BrutlColors.warning
            : ratio < 1.0 ? BrutlColors.accent
            : '#97C459';
          return (
            <View key={muscle} style={s.muscleRow}>
              ...
              <View style={[s.muscleTrack, ratio > 1.5 && s.muscleTrackWarn]}>
                <View style={[s.muscleFill, { width: `${Math.min(100, ratio * 100).toFixed(1)}%` as any, backgroundColor: fillColor }]} />
              </View>
              <BrutlText style={s.muscleSets}>{n}/{target}</BrutlText>
            </View>
          );
        })}
      </View>
      <BrutlText style={[s.annotLeft, { marginTop: 10, color: BrutlColors.textDisabled }]}>
        Week of {fmtShort(weekStart.getTime())} · {weekSessions} {weekSessions === 1 ? 'session' : 'sessions'} logged
      </BrutlText>
    </BrutlCard>
  );
}
...
function RecoverySection({
  ...
  vitals: { hrv: number | null; sleepHours: number | null; restingHR: number | null; recoveryScore: number | null };
  isAvailable: boolean;
  ...
  const rows = [
    { label: 'HRV', value: hrv, unit: 'ms', icon: 'pulse' as const, good: (v: number) => v >= 50 },
    { label: 'Sleep', value: sleepHours, unit: 'h', icon: 'moon' as const, good: (v: number) => v >= 7 },
    { label: 'Resting HR', value: restingHR, unit: 'bpm', icon: 'heart' as const, good: (v: number) => v < 65 },
    { label: 'Recovery', value: recoveryScore, unit: '%', icon: 'battery-charging' as const, good: (v: number) => v >= 70 },
  ...
  return (
    <BrutlCard>
      ...
      {prs.length === 0 ? (
        ...
        <>
          {visible.map((pr, i) => (
            <View key={pr.exercise} style={[s.prRow, i < visible.length - 1 && s.prRowBorder]}>
              <View style={s.prAccent} />
              <BrutlText style={[s.prExercise, { flex: 1 }]}>{pr.exercise}</BrutlText>
              <View style={{ alignItems: 'flex-end', gap: 2 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <BrutlText style={s.prWeight}>{pr.weightKg} kg × {pr.reps}</BrutlText>
                  {pr.isNew && (
          ...
          {prs.length > 8 && (
            <TouchableOpacity onPress={() => setShowAll((v) => !v)} style={{ marginTop: 10 }}>
    ...
    <BrutlCard>
      ...
      {last5.length === 0 ? (
        ...
        last5.map((r, i) => {
          ...
          return (
            <View key={r.id} style={[s.roastItem, i < last5.length - 1 && s.roastItemBorder]}>
              <BrutlText style={s.roastTrigger}>
                {TRIGGER_LABELS[r.triggerType] ?? r.triggerType} · {fmtShort(r.timestamp)}
              </BrutlText>
              <BrutlText style={s.roastBody}>{r.roastText}</BrutlText>
              {!!r.correctionText && (
                <BrutlText style={s.roastCorrection}>→ {r.correctionText}</BrutlText>
              )}
            </View>
          );
        })
...
function ConsistencySection({
  ...
  const cellColor = (state: DayState) =>
    state === 2 ? BrutlColors.accent : state === 1 ? '#2A0000' : BrutlColors.bgCardSubtle;
    ...
    <BrutlCard>
      ...
      <View style={s.streakRow}>
        <View style={s.streakStat}>
          <BrutlText style={[s.streakNum, { color: BrutlColors.accent }]}>{streakDays}</BrutlText>
          <BrutlText style={s.streakLabel}>{streakDays === 1 ? 'day streak' : 'days streak'}</BrutlText>
        </View>
        <View style={s.streakStat}>
          <BrutlText style={s.streakNum}>{workoutLogs.length}</BrutlText>
          <BrutlText style={s.streakLabel}>{workoutLogs.length === 1 ? 'workout' : 'workouts'}</BrutlText>
        </View>
        <View style={s.streakStat}>
      ...
      <View style={{ marginTop: 14, gap: 3 }}>
        {grid.map((week, wi) => (
          <View key={wi} style={{ flexDirection: 'row', gap: 3 }}>
            {week.map((state, di) => (
              <View key={di} style={[s.conCell, { flex: 1, backgroundColor: cellColor(state) }]} />
            ))}
          </View>
        ))}
      </View>
  ...
  return (
    <View style={s.container}>
      ...
      >
        ...
        <BodyWeightSection entries={weightEntries} addEntry={addWeightEntry} />

        <LiftProgression workoutLogs={workoutLogs} />

        <MuscleVolume workoutLogs={workoutLogs} />

        <RecoverySection vitals={vitals} isAvailable={isAvailable} hasPermission={hasPermission} />
...
const s = StyleSheet.create({
  ...
  rankGhost: {
    ...
    fontFamily: 'BebasNeue_400Regular',
    fontSize: 180, lineHeight: 180,
    opacity: 0.07, letterSpacing: -4,
  },
  ...
  rankXPNum:     { fontFamily: 'BebasNeue_400Regular', fontSize: 34, color: BrutlColors.accent, lineHeight: 36 },
  ...
  xpTrack:       { height: 4, backgroundColor: BrutlColors.border, borderRadius: 2, overflow: 'hidden' },
  xpFill:        { position: 'absolute', left: 0, top: 0, bottom: 0, backgroundColor: BrutlColors.accent, borderRadius: 2 },
  ...
  tile: {
    ...
    borderWidth: 0.5, borderColor: BrutlColors.border,
  },
  tileValue: { fontFamily: 'BebasNeue_400Regular', fontSize: 30, lineHeight: 32 },
  ...
  greenDot:  { width: 6, height: 6, borderRadius: 3, backgroundColor: BrutlColors.success },

  // ── Volume / annotations ──
  rangePill:         { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 999, borderWidth: 1, borderColor: BrutlColors.borderVisible },
  rangePillActive:   { backgroundColor: BrutlColors.accent, borderColor: BrutlColors.accent },
  rangePillText:     { fontSize: 10, color: BrutlColors.textMuted },
  rangePillTextActive: { color: '#000', fontWeight: '700' },
  annotRow:  { flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 },
  ...
  hmDayLabel:   { flex: 1, textAlign: 'center', fontSize: 9, color: BrutlColors.textDisabled },
  ...
  hmTooltip:    { marginTop: 8, padding: 8, backgroundColor: BrutlColors.bgCardSubtle, borderRadius: 6, borderWidth: 0.5, borderColor: BrutlColors.borderVisible },
  ...
  weightStatVal:  { fontSize: 13, color: BrutlColors.textPrimary, fontWeight: '600' },
  weightInputRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  weightInput:    { flex: 1, color: BrutlColors.textPrimary, fontSize: 16, borderBottomWidth: 1, borderBottomColor: BrutlColors.accent, paddingVertical: 4 },

  // ── Exercise pills ──
  exPill:          { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999, borderWidth: 1, borderColor: BrutlColors.borderVisible, backgroundColor: BrutlColors.bgCardSubtle },
  exPillActive:    { backgroundColor: BrutlColors.accent, borderColor: BrutlColors.accent },
  ...
  muscleTrack:     { flex: 1, height: 8, backgroundColor: BrutlColors.border, borderRadius: 4, overflow: 'hidden' },
  muscleTrackWarn: { borderWidth: 1, borderColor: BrutlColors.warning },
  muscleFill:      { position: 'absolute', left: 0, top: 0, bottom: 0, borderRadius: 4 },
  muscleSets:      { fontSize: 10, color: BrutlColors.textDisabled, width: 36, textAlign: 'right' },
  ...
  recoveryInsight:{ fontSize: 11, color: '#888888', fontStyle: 'italic', lineHeight: 17, marginTop: 12 },

  // ── PRs ──
  prRow:       { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, gap: 10 },
  prRowBorder: { borderBottomWidth: 0.5, borderBottomColor: BrutlColors.border },
  prAccent:    { width: 2, height: 28, backgroundColor: BrutlColors.accent, borderRadius: 1 },
  ...
  newBadge:    { backgroundColor: BrutlColors.accent, borderRadius: 999, paddingHorizontal: 5, paddingVertical: 1 },
  newBadgeText:{ fontSize: 8, color: '#000', fontWeight: '700', letterSpacing: 0.5 },

  // ── Roasts ──
  roastItem:       { paddingVertical: 10, gap: 5 },
  roastItemBorder: { borderBottomWidth: 0.5, borderBottomColor: BrutlColors.border },
  roastTrigger:    { fontSize: 10, color: BrutlColors.accent, letterSpacing: 1.5, fontWeight: '600' },
  roastBody:       { fontSize: 13, color: BrutlColors.textPrimary, lineHeight: 20 },
  roastCorrection: { fontSize: 12, color: BrutlColors.textDisabled, lineHeight: 18 },

  // ── Consistency ──
  streakRow:  { flexDirection: 'row', justifyContent: 'space-between' },
  streakStat: { flex: 1, alignItems: 'center', gap: 3 },
  streakNum:  { fontFamily: 'BebasNeue_400Regular', fontSize: 28, color: BrutlColors.textPrimary, lineHeight: 30 },
  streakLabel:{ fontSize: 9, color: BrutlColors.textDisabled, letterSpacing: 0.5 },
  conCell:    { height: 12, borderRadius: 2 },
  conLegend:  { flexDirection: 'row', gap: 12, marginTop: 10 },

  // ── Shared ──
  emptyNote: { fontSize: 11, color: BrutlColors.textDisabled, fontStyle: 'italic', lineHeight: 17 },
});
